...
const WHITELISTED_CHANNELS = [
  965284035985305689, 973699269557112882, 966835424603537448,
  965619953900326973, 970935992032825364, 970830900227100742,
];
...
export default async function run(
  ...
  const guild: Guild | undefined =
    client.guilds.cache.get(`965284035985305680`);
  if (reaction.partial) {
    try {
  ...
  if (!user.bot && !reaction.message.author?.bot) {
    ...
    if (
      reaction.emoji.name == "🔨" &&
      reaction.count < 2 &&
      WHITELISTED_CHANNELS.includes(Number(reaction.message.channelId)) &&
      !reaction.message.member?.roles.cache.some(
        (r) => r.id == "978201137972912198"
      )
    ) {
    ...
    if (
      reaction.emoji.name == "⭐" &&
      reaction.message.channelId !== "988887511843606598"
    ) {
      let reactionCount = reaction.count;
      if (
        reaction.message.reactions.cache
          .get("⭐")!
          .users.cache.has(reaction.message.author!.id)
      )
        reactionCount--;
