...
async function run(interaction: CommandInteraction) {
    switch (interaction.options.getSubcommand(true)) {
        ...
        case 'add':
            ...
            fs.writeFileSync('./src/db/blacklist.json', JSON.stringify(blacklist, null, 4));
...
const cmd = new SlashCommandBuilder()
    ...
    .addSubcommand(o => o.setName('add')
        ...
        .addStringOption(word => word.setName('word').setRequired(true).setDescription('The word to add'))
    )
    .addSubcommand(o => o.setName('remove')
        .setDescription('Removes a word from the blacklist')
        .addStringOption(word => word.setName('word').setRequired(true).setDescription('The word to remove'))
    )
